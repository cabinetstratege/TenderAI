...
export const calculateLocalScore = (
  ...
  if (profile.specialization) {
    const specKeywords = profile.specialization
      .toLowerCase()
      .split(/[\s,]+/)
      .filter((w) => w.length > 3);
  ...
  if (profile.cpvCodes) {
    const cpvs = profile.cpvCodes
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (cpvs.length > 0 && structuredCpvs.length > 0) {
      const set = new Set(structuredCpvs);
      const cpvMatches = cpvs.filter((cpv) => set.has(cpv)).length;
      if (cpvMatches > 0) {
        matchCount += cpvMatches;
        score += Math.min(20, cpvMatches * 6);
      }
    } else {
      cpvs.forEach((cpv) => {
        if (text.includes(cpv)) {
          matchCount += 1;
          score += 6;
        }
      });
  ...
  return { score: Math.max(5, Math.min(98, score)), matchCount };
};
...
const extractCPVs = (lot: any): string[] => {
  ...
  if (obj.CPV_SUPPLEMENTAIRE) {
    if (Array.isArray(obj.CPV_SUPPLEMENTAIRE)) {
      obj.CPV_SUPPLEMENTAIRE.forEach(
        (c: any) => c?.CODE_CPV && cpvs.push(c.CODE_CPV),
      );
...
const mapRecordToTender = (record: any, user: UserProfile): Tender => {
  ...
  const contact: TenderContact = {
    ...
    email: identity.MEL,
    phone: identity.TEL,
    address: [identity.ADRESSE, identity.CP, identity.VILLE]
      .filter(Boolean)
      .join(", "),
  ...
  const { score, matchCount } = calculateLocalScore(fullText, user, allLotCpvs);
  ...
  return {
    ...
    deadline: record.datelimitereponse
      ? record.datelimitereponse.split("T")[0]
      : "Non spécifiée",
...
export const tenderService = {
  getAuthorizedTenders: async (
    ...
    try {
      ...
      const blacklist = interactions
        .filter(
          (i) =>
            i.status === TenderStatus.BLACKLISTED ||
            i.status === TenderStatus.SAVED ||
            i.status === TenderStatus.WON ||
            i.status === TenderStatus.LOST,
        )
        .map((i) => i.tenderId);
      ...
      if (filters?.selectedRegion) {
        ...
        user.targetDepartments &&
        user.scope !== "France" &&
        user.scope !== "Europe"
      ) {
        targetDepts = user.targetDepartments
          .split(",")
          .map((d) => d.trim())
          .filter(Boolean);
      }
      ...
      if (filters?.searchTerm) {
        whereConditions.push(
          `search(objet, "${filters.searchTerm}") OR search(donnees, "${filters.searchTerm}")`,
        );
      ...
      if (filters?.procedureType) {
        whereConditions.push(
          `search(procedure_libelle, "${filters.procedureType}")`,
        );
      }
      ...
      if (!data.results || data.results.length === 0) {
        console.warn(
          "API returned 0 results. Falling back to MOCK DATA for demonstration.",
        );
        tenders = MOCK_TENDERS.map((t) => {
          ...
          return { ...t, compatibilityScore: score, matchCount: matchCount };
        });
      } else {
        tenders = data.results.map((record: any) =>
          mapRecordToTender(record, user),
        );
      }
  ...
  getCompetitorStats: async (user: UserProfile): Promise<MarketAnalysis> => {
        ...
        topCompetitors: [
          {
            ...
            winCount: 15,
            totalAmount: 4500000,
            topBuyer: "Métropole Lyon",
          },
          {
            name: "EIFFAGE",
            winCount: 12,
            totalAmount: 3200000,
            topBuyer: "Région Occitanie",
          },
          {
            name: "SNEF",
            winCount: 8,
            totalAmount: 1200000,
            topBuyer: "Hôpital Nord",
          },
        ],
    ...
    try {
      ...
      const competitorMap = new Map<
        string,
        { winCount: number; totalAmount: number; buyers: Map<string, number> }
      >();
      ...
      records.forEach((record: any) => {
        try {
          ...
          if (details.ATTRIBUTION) {
            if (Array.isArray(details.ATTRIBUTION))
              winners = details.ATTRIBUTION;
            else winners = [details.ATTRIBUTION];
          } else if (details.OBJET?.LOTS?.LOT) {
            const lots = Array.isArray(details.OBJET.LOTS.LOT)
              ? details.OBJET.LOTS.LOT
              : [details.OBJET.LOTS.LOT];
            lots.forEach((lot: any) => {
              if (lot.ATTRIBUTION) {
                if (Array.isArray(lot.ATTRIBUTION))
                  winners.push(...lot.ATTRIBUTION);
          ...
          winners.forEach((win) => {
            const nameRaw =
              win.TITULAIRE?.DENOMINATION || win.TITULAIRE || "Inconnu";
            const name = nameRaw
              .toUpperCase()
              .replace(/SAS|SARL|SA|EURL/g, "")
              .trim();
            ...
            if (win.MONTANT) {
              if (typeof win.MONTANT === "number") amount = win.MONTANT;
                ...
                amount = parseFloat(
                  win.MONTANT.replace(",", ".").replace(/[^0-9.]/g, ""),
                );
              }
            }
      ...
      const competitors: CompetitorStat[] = Array.from(
      ...
      return {
        topCompetitors: competitors
          ...
          .slice(0, 10),
        avgAwardAmount:
          awardCountWithAmount > 0
            ? Math.round(totalMarketAmount / awardCountWithAmount)
            : 0,
  ...
  getSavedTenders: async (): Promise<
    { tender: Tender; interaction: UserInteraction }[]
  > => {
    // DEMO MODE
    if (userService.isDemoMode()) {
      const local = localStorage.getItem("demo_interactions");
    ...
    const resultsPromise = workspaceInteractions.map(async (interaction) => {
      ...
      if (!tender) {
        try {
          ...
          console.error(
            "Failed to refetch missing tender",
            interaction.tenderId,
          );
        }
      }
    ...
    const results = (await Promise.all(resultsPromise)).filter(
      (item) => item !== null,
    ) as { tender: Tender; interaction: UserInteraction }[];
  ...
  updateInteraction: async (
    ...
    if (userService.isDemoMode()) {
      const local = localStorage.getItem("demo_interactions");
      let interactions: UserInteraction[] = local
        ? JSON.parse(local)
        : MOCK_INTERACTIONS.map((i) => i.interaction);

      const existingIdx = interactions.findIndex(
        (i) => i.tenderId === tenderId,
      );
      if (existingIdx >= 0) {
  ...
  getTenderById: async (
    id: string,
  ): Promise<{ tender: Tender; interaction?: UserInteraction } | null> => {
    // Demo Mode Fallback
    if (userService.isDemoMode()) {
