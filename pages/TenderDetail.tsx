...
const FormattedMessage = ({ text, role }: { text: string, role: string }) => {
    ...
    return (
        <div className={`text-sm leading-relaxed space-y-3 ${role === 'user' ? 'text-white' : 'text-slate-700 dark:text-slate-200'}`}>
            {paragraphs.map((para, idx) => {
                if (para.trim().startsWith('- ') || para.trim().startsWith('* ')) {
                     const items = para.split('\n');
                     return (
                         <ul key={idx} className={`list-disc pl-5 space-y-1 ${role === 'user' ? 'marker:text-blue-200' : 'marker:text-slate-400'}`}>
                             {items.map((item, i) => (
                                 <li key={i}>
                ...
                if (para.startsWith('#')) {
                    return <h3 key={idx} className={`font-bold mt-4 mb-2 text-base ${role === 'user' ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{parseBold(para.replace(/^#+ /, ''))}</h3>;
                }
                return (
                    <p key={idx}>
                        {para.split('\n').map((line, i) => (
...
END:VCALENDAR`;
  ...
  const handleDownloadTenderPDF = () => {
      ...
      doc.text(`ID BOAMP : ${data.tender.idWeb} | Date: ${new Date().toLocaleDateString()}`, margin, y);
      y += 15;
      ...
      // Clean description
      const cleanDesc = (data.tender.fullDescription || "").replace(/<[^>]*>?/gm, '');
  ...
  const scoreColor = tender.compatibilityScore >= 80 ? 'text-emerald-500 from-emerald-100 to-emerald-200 dark:from-emerald-500/20 dark:to-emerald-900/20 border-emerald-500/50' 
                   : tender.compatibilityScore >= 50 ? 'text-amber-500 from-amber-100 to-amber-200 dark:from-amber-500/20 dark:to-amber-900/20 border-amber-500/50'
             ...
             {isShareOpen && (
                 <div className="absolute right-0 top-full mt-2 w-64 bg-surface border border-border rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                     <div className="p-2 space-y-1">
                         <button onClick={handleSendEmail} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-sm text-slate-700 dark:text-slate-200 transition-colors text-left group">
                             <div className="p-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-md group-hover:bg-blue-200 dark:group-hover:bg-blue-900/50"><Mail size={16}/></div>
                             <div className="flex flex-col">
                                 <span className="font-medium">Envoyer par email</span>
                                 {userProfile?.contactEmail && <span className="text-[10px] text-slate-500 truncate max-w-[140px]">{userProfile.contactEmail}</span>}
                             </div>
                         </button>
                         <button onClick={handleAddToCalendar} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-sm text-slate-700 dark:text-slate-200 transition-colors text-left group">
                             <div className="p-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-md group-hover:bg-purple-200 dark:group-hover:bg-purple-900/50"><CalendarPlus size={16}/></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start h-[calc(100vh-140px)]">
        ...
        <div className="lg:col-span-6 space-y-6 h-full overflow-y-auto custom-scrollbar pr-2">
            <div className="glass-panel p-6 rounded-2xl shadow-xl space-y-8 relative overflow-hidden border border-white/20 dark:border-white/5">
                <div className="relative z-10 text-center">
                    <div className={`inline-flex flex-col items-center justify-center w-32 h-32 rounded-full border-4 bg-gradient-to-br shadow-[0_0_30px_rgba(0,0,0,0.1)] dark:shadow-[0_0_30px_rgba(0,0,0,0.3)] ${scoreColor} mb-4 relative`}>
                        <span className="text-4xl font-black tracking-tighter text-slate-800 dark:text-slate-100">{tender.compatibilityScore}%</span>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 transition-colors hover:border-slate-300 dark:hover:border-white/10">
                        <div className="p-2.5 bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-lg shadow-sm"><Building size={20} /></div>
                        <div className="min-w-0">
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-wide">Acheteur</p>
                            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">{tender.buyer}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 transition-colors hover:border-slate-300 dark:hover:border-white/10">
                        <div className={`p-2.5 rounded-lg shadow-sm ${daysRemaining < 5 ? 'bg-red-100 dark:bg-red-500/10 text-red-600 dark:text-red-400' : 'bg-slate-200 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400'}`}>
                            <Calendar size={20} />
                        </div>
                        <div className="min-w-0">
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-wide">Date Limite</p>
                            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{tender.deadline}</p>
                            <p className={`text-xs font-bold mt-0.5 ${daysRemaining < 5 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                    ...
                    <div className="pt-4 border-t border-border">
                        ...
                        <div className="space-y-3">
                             ...
                             {tender.contact.email && (
                                <a href={`mailto:${tender.contact.email}`} className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 transition-colors">
                                    <Mail size={14} /> <span className="truncate">{tender.contact.email}</span>
                                </a>
                             )}
                             {tender.contact.phone && (
                                <a href={`tel:${tender.contact.phone}`} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 hover:text-textMain transition-colors">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                     <a href={tender.linkDCE} target="_blank" rel="noreferrer" className="col-span-2 flex items-center justify-center gap-2 py-3 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 transition-all shadow-lg hover:border-primary/50">
                        <ExternalLink size={16} /> Voir sur BOAMP
                    </a>

                    {/* NEW: NOTES & REMINDERS DISPLAY */}
                    {interaction && (interaction.internalNotes || interaction.customReminderDate) && (
                        <div className="col-span-2 p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-xl space-y-2 mt-2">
                            ...
                            {interaction.customReminderDate && (
                                <div className="flex items-center gap-2 text-xs text-amber-700 dark:text-amber-200/70">
                                    <Clock size={12} />
                                    Rappel : {new Date(interaction.customReminderDate).toLocaleDateString()}
                                </div>
                            )}
                    ...
                    {interaction?.status === TenderStatus.TODO ? (
                        <button disabled className="col-span-2 py-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-900/50 text-blue-600 dark:text-blue-400 rounded-xl text-sm font-bold flex items-center justify-center gap-2">
                            <CheckCircle size={18}/> Dans le Pipeline
                        </button>
                    ) : (
                        <button onClick={() => handleStatusChange(TenderStatus.TODO)} className="col-span-1 flex items-center justify-center gap-2 py-3 bg-primary hover:bg-blue-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-500/20 dark:shadow-blue-900/20 transition-all">
                            <Save size={18} /> Sauvegarder
                        </button>
                    )}
                    
                    {interaction?.status !== TenderStatus.BLACKLISTED && (
                        <button onClick={() => handleStatusChange(TenderStatus.BLACKLISTED)} className="col-span-1 flex items-center justify-center gap-2 py-3 bg-white dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:border-red-200 dark:hover:border-red-900/30 rounded-xl text-sm font-bold transition-all">
        ...
        <div className="lg:col-span-6 flex flex-col h-full glass-panel rounded-2xl border border-white/20 dark:border-white/5 shadow-2xl overflow-hidden backdrop-blur-xl">
             ...
             <div className="flex p-2 bg-slate-50 dark:bg-slate-900/80 border-b border-border gap-1 shrink-0 overflow-x-auto">
                ...
                >
                    {activeTab === 'intelligence' && <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 dark:from-indigo-500/20 dark:to-purple-500/20"></div>}
                    <span className="relative flex items-center justify-center gap-2">
                        <Sparkles size={16} className={activeTab === 'intelligence' ? 'text-purple-600 dark:text-purple-400' : ''} /> 
                        Assistant IA
                    </span>
                </button>
             </div>
                    ...
                    <div className="prose prose-slate dark:prose-invert max-w-none">
                        <div className="flex flex-wrap gap-2 mb-6">
                            {tender.descriptors.map((tag, i) => (
                                <span key={i} className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md text-xs font-medium text-slate-600 dark:text-slate-300">
                                    {tag}
                                </span>
                            ))}
                        </div>
                 ...
                 <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-white/50 dark:bg-transparent">
                     <div className="grid grid-cols-1 gap-4">
                        {tender.lots.map((lot, idx) => (
                            <div key={idx} className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <h3 className="font-bold text-slate-900 dark:text-white text-sm leading-snug">{lot.title}</h3>
                                </div>
                                {lot.description && (
                                ...
                                {lot.cpv && lot.cpv.length > 0 && (
                                     <div className="mt-3 flex flex-wrap gap-1">
                                         {lot.cpv.map((c, i) => (
                                             <span key={i} className="text-[10px] font-mono text-slate-500 bg-slate-100 dark:bg-slate-900 px-1.5 py-0.5 rounded">
                                                 {c}
                                             </span>
                                         ))}
                                     </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar scroll-smooth">
                        ...
                        {!analysis ? (
                            <div className="bg-white dark:bg-slate-800/40 p-8 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 text-center space-y-4 mb-8 mx-4 mt-8">
                                ...
                                >
                                    {isAnalyzing ? <><Loader2 className="animate-spin" size={18}/> Analyse en cours...</> : "Lancer l'analyse"}
                                </button>
                            </div>
                        ) : (
                            <div className="space-y-4 mb-8 animate-in fade-in slide-in-from-top-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="bg-emerald-50/80 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-200/60 dark:border-emerald-500/20">
                                        <h4 className="font-bold text-emerald-700 dark:text-emerald-400 mb-3 flex items-center gap-2 text-sm"><CheckCircle size={16}/> Points Forts</h4>
                                        <ul className="space-y-2.5">
                                            {analysis.strengths.map((p, i) => (
                                                <li key={i} className="text-xs text-slate-700 dark:text-slate-300 flex gap-2 leading-relaxed">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0"></span> {p}
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                    <div className="bg-red-50/80 dark:bg-red-900/10 p-4 rounded-xl border border-red-200/60 dark:border-red-500/20">
                                        <h4 className="font-bold text-red-700 dark:text-red-400 mb-3 flex items-center gap-2 text-sm"><AlertTriangle size={16}/> Points de Vigilance</h4>
                                        <ul className="space-y-2.5">
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-6">
                            ...
                            >
                                <FileText size={20} className="text-blue-500 dark:text-blue-400 mx-auto mb-2 group-hover:scale-110 transition-transform"/>
                        ...
                        <div className="space-y-6 pb-2">
                            ...
                            {chatHistory.map((msg, idx) => (
                                <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2`}>
                                    <div className={`max-w-[90%] rounded-2xl px-5 py-4 text-sm shadow-md leading-relaxed relative group ${
                                        msg.role === 'user' 
                    ...
                    <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shrink-0">
                        ...
                        {chatHistory.length > 0 && (
                            <div className="flex justify-end gap-3 mt-3 px-1">
                                <button onClick={handleExportChatPDF} className="text-[10px] font-bold text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 flex items-center gap-1.5 transition-colors">
                                    <Download size={12} /> Exporter PDF
                                </button>
                                <button onClick={handleClearChat} className="text-[10px] font-bold text-slate-500 hover:text-red-600 dark:hover:text-red-400 flex items-center gap-1.5 transition-colors">
