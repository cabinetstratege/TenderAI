...
const Welcome: React.FC = () => {
  ...
  const renderStep1 = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-300">
        <div className="space-y-2 text-center">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">L'identité de votre entreprise</h2>
            <p className="text-slate-500 dark:text-slate-400">Pour personnaliser votre compagnon de veille.</p>
        </div>
        
        <div className="space-y-5">
            <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Nom de l'entreprise</label>
                <input 
                    type="text" 
            ...
            <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Site Web (Optionnel)</label>
                <input 
                    type="text" 
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600"
                    placeholder="https://..."
  ...
  const renderStep2 = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-300">
        <div className="space-y-2 text-center">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Apprenez-moi votre métier</h2>
            <p className="text-slate-500 dark:text-slate-400">Je configure les filtres BOAMP automatiquement pour vous.</p>
        </div>
        
        <div className="space-y-5">
            <div>
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Votre spécialisation</label>
                <textarea 
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600"
            ...
            {formData.cpvCodes && (
                <div className="bg-slate-100 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4 animate-in fade-in zoom-in">
                    <div>
                        <label className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase flex items-center gap-2"><Check size={14}/> Codes CPV Détectés</label>
                        <input 
                            type="text" 
                    ...
                    <div>
                        <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Mots-clés Négatifs (Anti-bruit)</label>
                        <textarea 
  ...
  const renderStep3 = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-300">
        <div className="space-y-2 text-center">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Votre terrain de jeu</h2>
            <p className="text-slate-500 dark:text-slate-400">Définissez votre zone d'intervention.</p>
        </div>
        ...
        <div className="space-y-2">
             <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Départements prioritaires (Si France)</label>
             <input 
                type="text" 
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600"
  ...
  if (isFinalizing) {
      return (
          <div className="min-h-screen bg-background flex flex-col items-center justify-center text-textMain p-4">
              ...
              <h2 className="text-2xl font-bold mb-2 text-textMain">Initialisation du Compagnon...</h2>
              <p className="text-slate-500 dark:text-slate-400 text-center max-w-md animate-pulse">
                  Recherche des opportunités pour <span className="text-blue-600 dark:text-blue-400 font-semibold">{formData.companyName}</span>...
              </p>
          </div>
      );
  }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="max-w-xl w-full relative z-10">
            ...
            <div className="flex items-center gap-3 justify-center mb-10">
                ...
                <span className="text-2xl font-bold text-textMain tracking-tight">Le Compagnon</span>
            </div>

            {/* Stepper */}
            <div className="flex justify-center mb-8 gap-3">
                {[1, 2, 3].map(i => (
                    <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${step >= i ? 'w-12 bg-primary shadow-glow' : 'w-4 bg-slate-200 dark:bg-slate-800'}`} />
                ))}
            </div>
