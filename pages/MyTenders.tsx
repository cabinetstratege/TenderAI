...
const KanbanCard: React.FC<KanbanCardProps> = ({ item, onNoteClick, onStatusChange }) => {
    ...
    return (
        <div className="bg-surface p-4 rounded-xl border border-border shadow-md hover:border-slate-400 dark:hover:border-slate-600 transition-all flex flex-col gap-3 group relative">
            ...
            <Link to={`/tender/${item.tender.id}`} className="font-semibold text-sm text-slate-800 dark:text-slate-200 line-clamp-2 hover:text-primary transition-colors">
                {item.tender.title}
            </Link>
            ...
            {item.interaction.internalNotes && (
                <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 p-2 rounded text-[10px] text-amber-700 dark:text-amber-200/80 truncate cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-950/40" onClick={() => onNoteClick(item.tender.id, item.interaction.internalNotes || '')}>
                    📝 {item.interaction.internalNotes}
                </div>
            )}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border mt-auto">
                ...
                <div className="flex gap-1">
                    ...
                    {item.interaction.status === TenderStatus.TODO && (
                        <button onClick={() => onStatusChange(item.tender.id, TenderStatus.IN_PROGRESS)} className="p-1.5 bg-slate-200 dark:bg-slate-800 hover:bg-primary text-slate-500 dark:text-slate-300 hover:text-white rounded transition-colors">
                            <ArrowRight size={14} />
                        </button>
                    )}
                    {item.interaction.status === TenderStatus.IN_PROGRESS && (
                        <button onClick={() => onStatusChange(item.tender.id, TenderStatus.SUBMITTED)} className="p-1.5 bg-slate-200 dark:bg-slate-800 hover:bg-primary text-slate-500 dark:text-slate-300 hover:text-white rounded transition-colors">
                            <ArrowRight size={14} />
                        </button>
                    )}
                    {item.interaction.status === TenderStatus.SUBMITTED && (
                       <div className="flex gap-1">
                            <button onClick={() => onStatusChange(item.tender.id, TenderStatus.WON)} className="p-1.5 bg-emerald-100 dark:bg-emerald-900/30 hover:bg-emerald-600 text-emerald-600 dark:text-emerald-400 hover:text-white rounded border border-emerald-200 dark:border-emerald-900/50" title="Gagné">
                                <CheckCircle size={14} />
                            </button>
                            <button onClick={() => onStatusChange(item.tender.id, TenderStatus.LOST)} className="p-1.5 bg-red-100 dark:bg-red-900/30 hover:bg-red-600 text-red-600 dark:text-red-400 hover:text-white rounded border border-red-200 dark:border-red-900/50" title="Perdu">
                                <X size={14} />
                            </button>
                       </div>
                    )}
    ...
    <div className="h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
        <div className="flex h-full gap-6 min-w-[1000px]">
            ...
            {/* COL 1: A QUALIFIER */}
            <div className="flex-1 min-w-[280px] flex flex-col bg-slate-100 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/50 rounded-2xl h-full">
                <div className="p-4 border-b border-slate-200 dark:border-slate-800/50 flex justify-between items-center bg-slate-200 dark:bg-slate-900/50 rounded-t-2xl">
                    <h3 className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2"><CheckCircle size={16} className="text-slate-500"/> À Qualifier</h3>
                    <span className="text-xs bg-white dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-full">{getColumnData(TenderStatus.TODO).length}</span>
                </div>
            ...
            {/* COL 2: EN REDACTION */}
            <div className="flex-1 min-w-[280px] flex flex-col bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-900/20 rounded-2xl h-full">
                <div className="p-4 border-b border-blue-200 dark:border-blue-900/20 flex justify-between items-center bg-blue-100 dark:bg-blue-900/20 rounded-t-2xl">
                    <h3 className="font-bold text-blue-800 dark:text-blue-100 flex items-center gap-2"><BrainCircuit size={16} className="text-blue-500"/> En Rédaction</h3>
                    <span className="text-xs bg-blue-200 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full border border-blue-300 dark:border-blue-500/30">{getColumnData(TenderStatus.IN_PROGRESS).length}</span>
                </div>
                <div className="p-3 flex-1 overflow-y-auto custom-scrollbar space-y-3">
            ...
            <div className="flex-1 min-w-[280px] flex flex-col bg-slate-100 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/50 rounded-2xl h-full">
                <div className="p-4 border-b border-slate-200 dark:border-slate-800/50 flex justify-between items-center bg-slate-200 dark:bg-slate-900/50 rounded-t-2xl">
                    <h3 className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2"><ArrowRight size={16} className="text-purple-500"/> Offre Soumise</h3>
                    <span className="text-xs bg-white dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-full">{getColumnData(TenderStatus.SUBMITTED).length}</span>
                </div>
                <div className="p-3 flex-1 overflow-y-auto custom-scrollbar space-y-3">
            ...
            {/* COL 4: TERMINE */}
            <div className="flex-1 min-w-[280px] flex flex-col bg-slate-100 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/50 rounded-2xl h-full opacity-80">
                <div className="p-4 border-b border-slate-200 dark:border-slate-800/50 flex justify-between items-center bg-slate-200 dark:bg-slate-900/50 rounded-t-2xl">
                    <h3 className="font-bold text-slate-600 dark:text-slate-400">Terminé</h3>
                    <span className="text-xs bg-white dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-full">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
             <div className="bg-surface border border-border rounded-xl shadow-2xl w-full max-w-md p-6 space-y-4 animate-in fade-in zoom-in duration-200">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => setEditingNoteId(null)}><X size={20} className="text-slate-500 hover:text-textMain"/></button>
                </div>
                <textarea 
                ...
                <div className="flex justify-end gap-2">
                    <button onClick={() => setEditingNoteId(null)} className="px-4 py-2 text-slate-500 text-sm font-medium hover:text-textMain">Annuler</button>
