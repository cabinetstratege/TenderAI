...
const RefreshButton: React.FC<RefreshButtonProps> = ({
  ...
  return (
    <button
      onClick={handleClick}
      disabled={isLoading}
      className={`flex items-center gap-2 px-4 py-2 bg-slate-800 border border-slate-700 hover:bg-slate-700 hover:text-white rounded-lg text-slate-300 text-sm font-medium transition-all cursor-pointer active:scale-95 ${isLoading ? "opacity-70 cursor-wait" : ""} ${className}`}
    >
