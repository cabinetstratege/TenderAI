...
const MultiSelect: React.FC<MultiSelectProps> = ({ 
  ...
  return (
    <div className="space-y-2" ref={containerRef}>
      ...
      <div className="min-h-[46px] p-2 border border-slate-300 dark:border-slate-700 rounded-lg focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent flex flex-wrap gap-2 bg-slate-50 dark:bg-slate-900/50">
        
        {selectedItems.map((item, idx) => (
            <span key={idx} className="bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm px-3 py-1 rounded-full flex items-center gap-1 border border-slate-200 dark:border-slate-700 shadow-sm">
                {item}
                <button 
        ...
        <div className="relative flex-1 min-w-[120px]">
            ...
            {showSuggestions && (inputValue || suggestions.length > 0) && filteredSuggestions.length > 0 && (
                <div className="absolute top-full left-0 w-full mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-20 max-h-48 overflow-y-auto">
                    {filteredSuggestions.map(suggestion => (
                        <div 
                            key={suggestion}
                            onClick={() => handleAddItem(suggestion)}
                            className="px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer text-sm text-slate-700 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700/50 last:border-0"
                        >
                            {suggestion}
                        </div>
                    ))}
