...
const MultiSelect: React.FC<MultiSelectProps> = ({ 
  ...
  return (
    <div className="space-y-2" ref={containerRef}>
      ...
      <div className="min-h-[46px] p-2 border border-slate-700 rounded-lg focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent flex flex-wrap gap-2 bg-background">
        
        {selectedItems.map((item, idx) => (
            <span key={idx} className="bg-slate-800 text-slate-200 text-sm px-3 py-1 rounded-full flex items-center gap-1 border border-slate-700">
                {item}
                <button 
        ...
        <div className="relative flex-1 min-w-[120px]">
            <input
                type="text"
                className="w-full h-full p-1 outline-none text-sm bg-transparent text-white placeholder-slate-600"
            ...
            {showSuggestions && (inputValue || suggestions.length > 0) && filteredSuggestions.length > 0 && (
                <div className="absolute top-full left-0 w-full mt-2 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-20 max-h-48 overflow-y-auto">
