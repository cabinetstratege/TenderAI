...
const LayoutNext: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isBlocking = isTrialExpired && pathname !== '/pricing' && pathname !== '/settings';

  return (
    <div className="flex h-screen bg-background text-textMain overflow-hidden relative selection:bg-indigo-500/30">
      <PaywallModal isOpen={isBlocking} onGoToPricing={() => router.push('/pricing')} />
      ...
      <aside className="hidden md:flex flex-col w-72 glass z-20 transition-all duration-300">
        ...
        <nav className="flex-1 space-y-1.5 px-4 mt-2">
          <NavItem href="/" icon={LayoutDashboard} label="Tableau de bord" id="tour-sidebar-dashboard" />
          <NavItem href="/my-tenders" icon={Briefcase} label="Mes Appels d'Offres" id="tour-sidebar-tenders" />
          <NavItem href="/stats" icon={BarChart2} label="Statistiques" />
          <NavItem href="/profile" icon={Building2} label="Profil Entreprise" id="tour-sidebar-profile" />
          <NavItem href="/pricing" icon={CreditCard} label="Abonnement" className={isTrialExpired ? 'animate-pulse text-primary font-bold' : ''} />
          ...
          <div className="px-4 pb-6">
            <Link
              ...
              className={`flex items-center gap-3 p-3 rounded-xl transition-all group border ${
                pathname === '/settings' ? 'bg-surface shadow-md border-border' : 'hover:bg-surface/50 border-transparent hover:border-border/50'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-textMain truncate leading-none mb-1 group-hover:text-primary transition-colors">{userEmail.split('@')[0]}</p>
                <p className="text-[10px] text-textMuted font-medium truncate">{isTrialExpired ? 'Abonnement Expiré' : 'Gérer mon compte'}</p>
              </div>
            </Link>
          </div>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-background/95 backdrop-blur-xl z-40 pt-24 px-6 md:hidden animate-fade-in flex flex-col">
          {profile && profile.subscriptionStatus === 'Trial' && profile.trialStartedAt && <TrialTimer startDate={profile.trialStartedAt} />}
          <nav className="space-y-3">
            <div onClick={() => setIsMobileMenuOpen(false)}>
              <NavItem href="/" icon={LayoutDashboard} label="Tableau de bord" />
            </div>
            <div onClick={() => setIsMobileMenuOpen(false)}>
              <NavItem href="/my-tenders" icon={Briefcase} label="Mes Appels d'Offres" />
            </div>
            <div onClick={() => setIsMobileMenuOpen(false)}>
              <NavItem href="/stats" icon={BarChart2} label="Statistiques" />
            </div>
            <div onClick={() => setIsMobileMenuOpen(false)}>
              <NavItem href="/profile" icon={Building2} label="Profil Entreprise" />
            </div>
            <div onClick={() => setIsMobileMenuOpen(false)}>
            ...
            {isSuperAdmin && (
              <div onClick={() => setIsMobileMenuOpen(false)}>
                <NavItem href="/super-admin" icon={ShieldCheck} label="Super Admin" className="text-emerald-400" />
              </div>
            )}
      ...
      >
        <div className="container mx-auto p-4 md:p-8 max-w-7xl min-h-[calc(100vh-2rem)]">{children}</div>
      </main>
    </div>
  );
};
