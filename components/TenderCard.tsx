...
const TenderCard: React.FC<TenderCardProps> = ({
  ...
  const getScoreColor = (score: number) => {
    if (score >= 80)
      return "text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-500/20";
    if (score >= 50)
      return "text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-950/40 border-amber-200 dark:border-amber-500/20";
  ...
  const matchedKeywords = useMemo(() => {
    ...
    const userKeywords = userProfile.specialization
      .toLowerCase()
      .split(/[\s,]+/)
  ...
  return (
    <div
      ...
      className={`bg-surface rounded-2xl border transition-all duration-300 flex flex-col h-full group relative overflow-hidden cursor-pointer ${
        isVisited
      ...
      <div className="p-5 flex-1 flex flex-col relative z-10">
        <div className="flex justify-between items-start mb-3">
          <div className="flex gap-2 items-center">
            <span
              className={`text-[10px] font-bold px-2.5 py-1 rounded-full border tracking-wide uppercase ${getScoreColor(tender.compatibilityScore)}`}
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="cursor-auto text-slate-400 hover:text-blue-500 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
        ...
        <button
          ...
          className="cursor-auto block text-left w-full group/title"
        >
          <h3
          ...
          <div className="mb-4 flex items-start gap-2 overflow-hidden">
            ...
            <div className="flex flex-wrap gap-1.5">
              {matchedKeywords.map((kw, i) => (
                <span
                  key={i}
                  className="text-[10px] font-bold text-blue-700 dark:text-blue-200 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-500/20 capitalize"
                >
                  {kw}
                </span>
              ))}
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-[color:var(--color-surface-highlight)] dark:bg-[color:var(--color-surface)] space-y-3 relative z-10">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              onStatusChange(tender, TenderStatus.SAVED);
            }}
            className="cursor-auto flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-bold text-white bg-slate-900 dark:bg-blue-600 hover:bg-slate-800 dark:hover:bg-blue-500 shadow-md transition-all"
          >
