...
import { Loader2, TrendingUp, Activity, Target, Award, Wallet, Filter, Trophy, Swords, Building } from 'lucide-react';

type StatsScreenProps = {
  onNavigateTender?: (id: string) => void;
};

const StatsScreen: React.FC<StatsScreenProps> = ({ onNavigateTender }) => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const totalReceived = 200;
    const totalConsulted = visitedIds.length;
    const totalSaved = filteredData.length;

    const totalBudget = filteredData.reduce((acc, { tender }) => acc + (tender.estimatedBudget || 0), 0);
    const avgScore = totalSaved > 0 ? Math.round(filteredData.reduce((acc, { tender }) => acc + tender.compatibilityScore, 0) / totalSaved) : 0;

    const winnableCount = filteredData.filter(({ tender, interaction }) => tender.compatibilityScore >= 70 || interaction.status === TenderStatus.WON).length;
      ...
      <div className="bg-surface/50 p-2 rounded-2xl border border-white/5 backdrop-blur-sm flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex p-1 bg-slate-900 rounded-xl">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'internal' ? 'bg-primary text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
          >
        ...
        <div className="flex gap-2">
          <div className="flex items-center bg-slate-900 rounded-lg p-1 border border-slate-700">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${period === '30d' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}
            >
              30J
            </button>
            <button
              onClick={() => setPeriod('year')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${period === 'year' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}
            >
              Année
            </button>
            <button
              onClick={() => setPeriod('all')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${period === 'all' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}
            >
              Tout
            </button>
          </div>
                ...
                {stats.funnelData.map((step: any, idx: number) => (
                  <div key={idx} className="relative">
                    ...
                    <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${(step.value / stats.kpi.totalConsulted) * 100}%`, backgroundColor: step.fill }}></div>
                    </div>
                  </div>
                ))}
                <div className="mt-4 p-3 bg-slate-800/50 rounded-lg text-xs text-slate-400 text-center">Vous transformez <strong>{stats.conversionRate}%</strong> des dossiers traités.</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
          <div className="bg-gradient-to-r from-slate-900 to-indigo-900/30 p-6 rounded-2xl border border-indigo-500/30 flex items-center justify-between">
            <div>
              ...
              <p className="text-indigo-200 text-sm mt-1">Basé sur {marketStats.totalAwardsAnalyzed} avis d'attribution récents liés à "{userProfile?.specialization}"</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex justify-between mt-1">
                        <span className="text-[10px] text-slate-500 flex items-center gap-1">
                          <Building size={10} /> Client Principal: {comp.topBuyer}
                        </span>
            ...
            <div className="bg-surface p-6 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col">
              ...
              <div className="flex-1 w-full h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={marketStats.topCompetitors.slice(0, 5)} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="totalAmount" nameKey="name">
              ...
              <p className="text-center text-xs text-slate-500 mt-4 italic">Données basées sur les montants attribués déclarés publiquement.</p>
            </div>
          </div>
        </div>
      )}
...
const KPICard = ({ title, value, icon: Icon, color, trend, trendColor }: any) => (
  <div className="bg-surface p-5 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col justify-between h-full relative overflow-hidden group hover:border-slate-600 transition-all">
    ...
    <div className="flex justify-between items-start mb-3 relative z-10">
      ...
      {trend && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${trendColor}`}>{trend}</span>}
    </div>
