...
import { LogOut, Download, Database, Shield, PlayCircle, Moon, Sun, Bell, BellOff, Trash2, CheckCircle, RefreshCcw } from 'lucide-react';
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onLogout, onRestartTutorial }) => {
    ...
    <div className="space-y-8 max-w-4xl mx-auto pb-12">
      <div className="flex justify-between items-center border-b border-border pb-6">
        <div>
          ...
          <p className="text-textMuted text-sm mt-1">Personnalisez votre expérience et gérez vos données.</p>
        </div>
      </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-1 flex gap-1 max-w-md">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-sm font-medium transition-all ${
              theme === 'light' ? 'bg-white text-slate-900 shadow-sm border border-slate-200' : 'text-textMuted hover:text-textMain'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-sm font-medium transition-all ${
              theme === 'dark' ? 'bg-slate-700 text-white shadow-sm' : 'text-textMuted hover:text-textMain'
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-surface p-5 rounded-xl border border-border flex justify-between items-center">
            <div className="flex gap-3 items-center">
              <div className={`p-2 rounded-lg ${notifEnabled ? 'bg-primary/10 text-primary' : 'bg-slate-700/50 text-slate-500'}`}>
                {notifEnabled ? <Bell size={20} /> : <BellOff size={20} />}
              </div>
              <div>
                <h4 className="font-bold text-textMain text-sm">Notifications</h4>
                <p className="text-xs text-textMuted">Alertes échéances et rappels.</p>
              </div>
            </div>
          ...
          <div className="bg-surface p-5 rounded-xl border border-border flex justify-between items-center">
            <div className="flex gap-3 items-center">
              ...
              <div>
                ...
                <p className="text-xs text-textMuted">Forcer la mise à jour des AO.</p>
              </div>
            </div>
            <button
              onClick={handleClearCache}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                cacheCleared ? 'bg-green-500/10 text-green-500 border-green-500/30' : 'bg-surfaceHighlight text-textMain border-border hover:bg-slate-700 hover:text-white'
              }`}
            >
              {cacheCleared ? (
          ...
          <div className="bg-surface p-5 rounded-xl border border-border flex justify-between items-center md:col-span-2">
            <div className="flex gap-3 items-center">
              ...
              <div>
                <h4 className="font-bold text-textMain text-sm">Export Données (GDPR)</h4>
                <p className="text-xs text-textMuted">Télécharger toutes vos données personnelles au format JSON.</p>
              </div>
            </div>
            <button
      ...
      <div className="text-center text-xs text-textMuted pt-8">
        ...
        <p className="mt-1">ID Session: {Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
      </div>
    </div>
  );
};
